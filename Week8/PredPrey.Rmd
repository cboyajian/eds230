...
```{r}
# initial conditions (has to be list since there are two initial conditions)
...
currpop <- c(prey = 1, pred = 1)
days <- seq(from = 1, to = 500)
allresults <- as.data.frame(sens_pp) %>% 
...
ggplot(allresl, 
       aes(metric, pop, 
           col = metric)) + 
  geom_boxplot() + 
  facet_wrap(~metric, scales = "free") # make scales free to see better
...
ggplot(allresl, 
       aes(pop, 
           col = metric)) + 
  stat_ecdf(geom = "line") + # calculate partial regression coefficients
...
res <- ode(func = lorenz, c(x = 0.1, y = 0, z = 0), 
...
ggplot(as.data.frame(res), 
       aes(y, z, col = time)) + geom_point()
```


```{r lorenze}
ressimple <- as.data.frame(res) %>% 
  gather(key = "var", value = "value", -time)
...
res <- ode(func = lorenz, c(x = 0.3, y = 5, z = 10), 
           times = seq(0,50, by = 0.01),  
           parms = pars)
