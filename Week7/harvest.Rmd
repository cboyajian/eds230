...
```{r harvest}
# what if we harvest at a much greater rates
...
res <- harvestr %>% 
  map_dfc(~getcarbon(Pinitial = Pinitial, tm = tm, 
                     K = 100, r = 0.2, hfun = dharvest, harv = .x))
...
resl <- res %>% 
  pivot_longer(1:17, names_to = "harvestr",
               values_to = "carbon")
...
r <- rnorm(mean = 0.3, sd = 0.05, n = np) # growth rate
harv <- runif(min = 0.0, max = 0.4, n = np) # harvest rate
# don't vary K (don't have to vary all parameters)
...
#   pmap(p_wrapper, K = 100, Pinitial = Pinitial, simtimes = tm, func = dharvest)
# 
# # extract out results from pmap into a data frame
# allres <- allresults %>% 
#   map_dfr(`[`, c("C10", "C50"))

# use `pmap_dfr` instead of `pmap` and `map_dfr` (might work)
allres <- as.data.frame(sens_forest$X) %>%
  pmap_dfr(p_wrapper, K = 100, Pinitial = Pinitial, simtimes = tm, func = dharvest)
...
# probably want to harvest at 10% to be sure that you're always okay
# harvest rate at 20% is okay if you have higher growth, but if you have a slow growth rate
```
