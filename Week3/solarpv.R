...
solarpv = function(area, eff = 0.8, PR = 0.75, solar, clr = "blue", 
                   eunits = "J", etype = "both", g = TRUE, ethresh = 10000) {
  ...
  { result = ifelse((x > ethresh), 
                    eff * x, 
                    x * eff * (max(0, x / ethresh)))
    return(result)
  }
  ...
  return(list(annual = annualsolar[,c("year","elect")], 
              mean = mean(annualsolar$elect)))
