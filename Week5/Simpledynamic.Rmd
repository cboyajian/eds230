...
# differential function for population growth rate
# include time here but don't use it; we will need this later
# important for tools we'll use later for numerical integration
source("dexppop.R")

# see how it works
dexppop(P = 20, r = 0.01)
# output is the rate of change at that specific moment in time

# notices this is the same as
dexppop(t = 100, P = 20, r = 0.01)

# look at this for a range of initial populations
...
```{r explicit}
# function given analytical integration (solution)
...
Ptime = years %>% 
  map_dbl(~exppop(P0 = initialrabbits, r = 0.01, K = 1000, T = .x))

# keep track of what times we ran
Ptime = data.frame(P = Ptime, years = years)

ggplot(Ptime, aes(x = years, 
                  y = P)) + 
  geom_point() + 
  labs(x = "years",
       y = "Rabbit Population")
